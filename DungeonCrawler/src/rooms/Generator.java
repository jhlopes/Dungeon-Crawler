...
public class Generator {
	...
	private int[] generateMap() {
		...
		if (ctr.OOB(startingRow, startingColumn))
			return null;
	...
	private void addRandomRoom(int row, int column) {
		...
		for (int i = 0; i < freeSpaces.length; i++) {
			if (freeSpaces[i]) {
				...
				addRandomRoom(Direction.intDirection(i).getNextPositionRow(row),
						Direction.intDirection(i).getNextPositionColumn(column));
			}
		}
	}
